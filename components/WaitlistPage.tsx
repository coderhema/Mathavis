...
const WaitlistPage: React.FC = () => {
  ...
  const waitlistEndpoint = (import.meta as any).env?.VITE_WAITLIST_SPREADSHEET_URL as string | undefined;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!waitlistEndpoint) {
      setStatus({
        type: 'error',
        message: 'Waitlist endpoint is not configured yet. Add VITE_WAITLIST_SPREADSHEET_URL in Vercel to enable submissions.',
      });
      return;
    }
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message: 'You are on the waitlist. We saved your details to the configured spreadsheet endpoint.',
      });
      setName('');
      setAgeGroup('');
      setRole('');
            ...
            <div className="relative overflow-visible rounded-[36px] border-4 border-slate-200 bg-white p-6 pb-24 shadow-[0_18px_0_#e2e8f0] dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_18px_0_#0f172a] sm:p-8">
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <p className="text-xs font-medium leading-relaxed text-slate-400 dark:text-slate-500">
                  Submissions are sent to the URL stored in VITE_WAITLIST_SPREADSHEET_URL.
                </p>
              </form>
